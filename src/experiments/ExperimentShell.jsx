  ...
  return (
    <section className="card">
      <h3 className="font-semibold text-slate-900 mb-3 flex items-center gap-2">
        <span className="w-6 h-6 rounded-full bg-blue-100 text-blue-700 text-xs font-bold flex items-center justify-center shrink-0">{n}</span>
        {title}
      </h3>
      {children}
    </section>
...
export default function ExperimentShell({ experiment }) {
  ...
  // Readings table: up to the last 8 samples (downsampled).
  const rows = useMemo(() => {
  ...
  // Section numbering (sequential, accounting for the optional Data section).
  let n = 0
  const objN = ++n, methodN = ++n, simN = ++n
  const dataN = experiment.series ? ++n : null
  const reportN = experiment.report ? ++n : null
  const checkN = ++n
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      {/* Three-column dashboard on large screens; stacks on smaller screens.
          Col 1: objective + method. Col 2: the experiment. Col 3: data, report & check. */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        {/* Column 1 — Objective & Setup */}
        <div className="lg:col-span-3 flex flex-col gap-5">
          <Section n={objN} title={t('lab_sec_objective')}>
            <p className="text-slate-700 text-sm leading-relaxed">{pick(experiment.objective)}</p>
          </Section>
          <Section n={methodN} title={t('lab_sec_method')}>
        ...
        {/* Column 2 — The experiment */}
        <div className="lg:col-span-5">
          <Section n={simN} title={t('lab_sec_sim')}>
            {sim}
          </Section>
        </div>

        {/* Column 3 — Data, report & comprehension */}
        <div className="lg:col-span-4 flex flex-col gap-5">
          {experiment.series && (
            <Section n={dataN} title={t('lab_sec_data')}>
                  ...
                  <table className="w-full text-sm border-collapse">
                    <thead>
                      <tr className="bg-slate-100">
                        <th className="border border-slate-200 px-2 py-1.5 text-left text-blue-700">{t('lab_col_time')}</th>
                        {experiment.series.map(s => <th key={s.key} className="border border-slate-200 px-2 py-1.5 text-left text-blue-700">{pick(s.label)}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                      {rows.map((r, i) => (
                        <tr key={i} className={i % 2 ? 'bg-slate-50' : 'bg-white'}>
                          <td className="border border-slate-200 px-2 py-1.5 text-slate-700">{r.t}</td>
                          {experiment.series.map(s => <td key={s.key} className="border border-slate-200 px-2 py-1.5 text-slate-700">{r[s.key] ?? '—'}</td>)}
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {experiment.report && (
            <Section n={reportN} title={t('lab_sec_report')}>
              {report
          ...
          <Section n={checkN} title={t('lab_sec_check')}>
